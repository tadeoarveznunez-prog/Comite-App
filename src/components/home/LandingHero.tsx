  ...
  return (
    <div className="space-y-16 pb-16">
      ...
      <section className="relative overflow-hidden pt-12 pb-16 px-4 sm:px-6 lg:px-8 max-w-6xl mx-auto text-center">
        ...
        <div className="mt-6 pt-6 border-t border-slate-200/60 dark:border-slate-800/80 max-w-md mx-auto">
          <span className="text-[11px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider block mb-2">
            Acceso Rápido de Configuración
          </span>
          <div className="flex items-center justify-center">
            <button
              type="button"
              disabled={demoLoading}
              onClick={() => handleQuickDemoLogin('admin@votoescolar.edu', 'Admin123!')}
              className="px-4 py-2 rounded-xl bg-indigo-50 hover:bg-indigo-100 dark:bg-indigo-950/60 dark:hover:bg-indigo-900 text-xs font-bold text-indigo-700 dark:text-indigo-300 transition-colors border border-indigo-200/80 dark:border-indigo-800 flex items-center gap-2"
            >
      ...
      <section className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
        ...
        {elections.length === 0 ? (
          <div className="p-8 text-center bg-white dark:bg-slate-800/80 rounded-3xl border border-slate-200/80 dark:border-slate-800 shadow-2xs">
            <Vote className="w-10 h-10 mx-auto text-slate-400 mb-2" />
            <p className="text-sm font-bold text-slate-700 dark:text-slate-300">
              No hay elecciones activas en este momento
            </p>
            <p className="text-xs text-slate-500 mt-1 max-w-md mx-auto">
              El administrador del colegio puede ingresar y configurar la primera votación o cargar la nómina de estudiantes.
            </p>
          </div>
        ) : (
